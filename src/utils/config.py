    ...
    if hasattr(cfg, "optim"):
        ...
        if hasattr(cfg.optim, "weight_decay"):
            if cfg.optim.weight_decay < 0:
                raise ValueError(
                    f"optim.weight_decay must be non-negative, got {cfg.optim.weight_decay}"
                )
    ...
    if hasattr(cfg, "attack"):
        if hasattr(cfg.attack, "epsilon"):
            ...
            if cfg.attack.epsilon > 1:
                raise ValueError(
                    f"attack.epsilon should typically be <= 1, got {cfg.attack.epsilon}"
                )
    ...
    if hasattr(cfg, "data"):
        ...
        if hasattr(cfg.data, "num_workers"):
            if cfg.data.num_workers < 0:
                raise ValueError(
                    f"data.num_workers must be non-negative, got {cfg.data.num_workers}"
                )
    ...
    if hasattr(cfg, "uat"):
        ...
        if hasattr(cfg.uat, "pair_mode"):
            ...
            if cfg.uat.pair_mode not in valid_modes:
                raise ValueError(
                    f"uat.pair_mode must be one of {valid_modes}, " f"got {cfg.uat.pair_mode}"
                )
