...
def verify_attacks(num_batches=5):
    ...
    *_, test_loader = get_cifar10_loaders(batch_size=128, num_workers=0)
    ...
    for batch_idx, (x, y) in enumerate(test_loader):
        ...
        logger.info(
            f"  Clean: Acc={clean['accuracy']:.1f}%, Conf={clean['confidence']:.4f}, Ent={clean['entropy']:.4f}, Margin={clean['margin']:.4f}"
        )
        ...
        logger.info(
            f"  PGD:   Acc={pgd_m['accuracy']:.1f}%, Conf={pgd_m['confidence']:.4f}, Ent={pgd_m['entropy']:.4f}, Loss={pgd_m['loss']:.4f}"
        )
        ...
        logger.info(
            f"  ConfS: Acc={conf_m['accuracy']:.1f}%, Conf={conf_m['confidence']:.4f}, Ent={conf_m['entropy']:.4f}, Margin={conf_m['margin']:.4f}"
        )
        ...
        logger.info(
            f"  Ambig: Acc={amb_m['accuracy']:.1f}%, Conf={amb_m['confidence']:.4f}, Ent={amb_m['entropy']:.4f}, Margin={amb_m['margin']:.4f}"
        )

    # Average metrics
    def avg_metrics(metrics_list):
        return {
            k: sum(m[k] for m in metrics_list) / len(metrics_list) for k in metrics_list[0].keys()
        }
    ...
    logger.info("\nAverage Metrics:")
    logger.info(
        f"{'Attack':<15} {'Accuracy':<12} {'Confidence':<12} {'Entropy':<12} {'Margin':<12} {'Loss':<12}"
    )
    logger.info("-" * 84)
    logger.info(
        f"{'Clean':<15} {clean_avg['accuracy']:>10.1f}% {clean_avg['confidence']:>11.4f} {clean_avg['entropy']:>11.4f} {clean_avg['margin']:>11.4f} {clean_avg['loss']:>11.4f}"
    )
    logger.info(
        f"{'PGD':<15} {pgd_avg['accuracy']:>10.1f}% {pgd_avg['confidence']:>11.4f} {pgd_avg['entropy']:>11.4f} {pgd_avg['margin']:>11.4f} {pgd_avg['loss']:>11.4f}"
    )
    logger.info(
        f"{'ConfSmooth':<15} {conf_avg['accuracy']:>10.1f}% {conf_avg['confidence']:>11.4f} {conf_avg['entropy']:>11.4f} {conf_avg['margin']:>11.4f} {conf_avg['loss']:>11.4f}"
    )
    logger.info(
        f"{'ClassAmbiguity':<15} {amb_avg['accuracy']:>10.1f}% {amb_avg['confidence']:>11.4f} {amb_avg['entropy']:>11.4f} {amb_avg['margin']:>11.4f} {amb_avg['loss']:>11.4f}"
    )
    ...
    logger.info(
        f"   Clean conf: {clean_avg['confidence']:.4f}, ConfSmooth conf: {conf_avg['confidence']:.4f}"
    )
    logger.info(
        f"   Reduction: {(clean_avg['confidence'] - conf_avg['confidence']) / clean_avg['confidence'] * 100:.1f}%"
    )
    ...
    total_checks += 1
    conf_maintains_accuracy = (
        abs(conf_avg["accuracy"] - clean_avg["accuracy"]) < 0.1
    )  # Allow tiny numerical difference
    ...
    logger.info(
        f"   Clean entropy: {clean_avg['entropy']:.4f}, ConfSmooth entropy: {conf_avg['entropy']:.4f}"
    )
    logger.info(
        f"   Increase: {(conf_avg['entropy'] - clean_avg['entropy']) / clean_avg['entropy'] * 100:.1f}%"
    )
    ...
    logger.info(
        f"   Clean conf: {clean_avg['confidence']:.4f}, Ambiguity conf: {amb_avg['confidence']:.4f}"
    )
    logger.info(
        f"   Reduction: {(clean_avg['confidence'] - amb_avg['confidence']) / clean_avg['confidence'] * 100:.1f}%"
    )
    ...
    total_checks += 1
    amb_maintains_accuracy = (
        abs(amb_avg["accuracy"] - clean_avg["accuracy"]) < 0.1
    )  # Allow tiny numerical difference
    status = "✅ PASS" if amb_maintains_accuracy else "❌ FAIL"
    logger.info(f"\n6. ClassAmbiguity maintains predictions (same as clean): {status}")
    logger.info(
        f"   Clean: {clean_avg['accuracy']:.1f}%, ClassAmbiguity: {amb_avg['accuracy']:.1f}%"
    )
    ...
    logger.info(
        f"   Clean margin: {clean_avg['margin']:.4f}, Ambiguity margin: {amb_avg['margin']:.4f}"
    )
    logger.info(
        f"   Reduction: {(clean_avg['margin'] - amb_avg['margin']) / clean_avg['margin'] * 100:.1f}%"
    )
